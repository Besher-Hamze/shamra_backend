...
export class NotificationsController {
    ...
    @Get('my')
    async findMyNotifications(@Query() query: NotificationQueryDto, @Request() req) {
        const result = await this.notificationsService.findByRecipient(req.user.sub, query);
        return {
            success: true,
            message: 'تم جلب إشعاراتك بنجاح',
            data: result.data,
            pagination: result.pagination,
        };
    }
    ...
    async createSystemNotification(
        ...
        const notifications = await this.notificationsService.createSystemNotification(
            systemNotificationDto.title,
            systemNotificationDto.titleAr,
            systemNotificationDto.message,
            systemNotificationDto.messageAr,
            systemNotificationDto.type,
            systemNotificationDto.recipientIds,
            systemNotificationDto.metadata,
        );
        return {
            success: true,
            message: `تم إنشاء ${notifications.length} إشعار نظام بنجاح`,
            data: notifications,
        };
    }
}
